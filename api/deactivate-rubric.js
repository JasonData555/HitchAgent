 ...
 *
 * Permanently deletes a Rubric HTML document from Vercel Blob Storage,
 * immediately invalidating the URL. Called by an Airtable automation when
 * a Program Manager checks the "Deactivate Rubric" checkbox on a Rubric record.
 *
 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Table:   Rubric
 * Trigger: "Deactivate Rubric" checkbox field is checked
 *
 * Step 1 — Find record
 *   Find the Rubric record that triggered the automation.
 *
 * Step 2 — Retrieve rubric URL
 *   Read the "rubric_url" field value from that record.
 *
 * Step 3 — Guard: stop if no URL
 *   If "rubric_url" is empty, stop the automation — there is nothing to delete.
 *
 * Step 4 — Call deactivate-rubric endpoint
 *   Send an HTTP POST request to:
 *     https://<your-vercel-domain>/api/deactivate-rubric
 *   Headers:
 *     Content-Type: application/json
 *     x-api-key: <INTERNAL_API_KEY>
 *   Body:
 *     { "blobUrl": "<rubric_url field value>" }
 *
 * Step 5 — On success (response.success === true)
 *   Set "Rubric URL Status" to "Deactivated" on the Rubric record.
 *   Do NOT clear or modify "rubric_url" — preserve it for audit purposes.
 *
 * Step 6 — On failure (response.success === false)
 *   Write response.error to the "Rubric Deactivation Log" field on the Rubric
...
export default async function handler(req, res) {
  ...
  log('request_received', { endpoint: 'deactivate-rubric', blobUrl: actualBlobUrl });
