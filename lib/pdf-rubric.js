 ...
 * Always-visible items (alwaysShow: true) render with "—" when empty.
 * Optional items are omitted when empty.
 */
function renderContextBar(items) {
  const visible = items.filter(item => item.alwaysShow || (item.value && item.value.trim()));
  if (visible.length === 0) return '';

  const itemsHtml = visible.map((item, i) => {
    const isLast = i === visible.length - 1;
    const displayValue = (item.value && item.value.trim()) ? item.value : '—';
    return `
      <div class="context-item${isLast ? ' context-item-last' : ''}">
        <div class="context-label">${esc(item.label)}</div>
        <div class="context-value">${esc(displayValue)}</div>
      </div>`;
  }).join('\n');
...
export function buildRubricDocument({
  ...
  const contextBarHtml = renderContextBar([
    { label: 'POSITION',           value: searchName,       alwaysShow: true },
    { label: 'LOCATION',           value: location,         alwaysShow: true },
    { label: 'CURRENT TEAM',       value: currentTeamSize,  alwaysShow: true },
    { label: 'EST. TEAM 18-24 MO', value: teamSize18Months, alwaysShow: true },
  ]);
