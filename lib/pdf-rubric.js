// DEPRECATED: This library generated the matrix scoring PDF rubric.
// The rubric format has changed to a hosted HTML document. See lib/html-rubric.js for the replacement.
...
function buildRubricHtml({
  ...
  // Values come from Airtable fields (editable by the PM before PDF generation).
  // PMs may bold specific lines using **text** markdown syntax. Bold lines
  // render without a bullet point prefix.

  function parsePriorityField(text) {
    if (!text || text.trim() === '' || text.trim().toLowerCase() === 'none') {
      return null;
    }
    // Normalize backslash-escaped asterisks (\* → *) so that Airtable's
    // markdown escaping does not prevent bold detection.
    const normalized = text.replace(/\\\*/g, '*');
    const lines = normalized.split('\n').map(s => s.trim()).filter(s => s.length > 0);
    if (lines.length === 0) return null;
    return lines.map(line => {
      // Full-line bold: **text** (entire line wrapped in **)
      const fullBold = line.match(/^\*\*(.+)\*\*$/);
      if (fullBold) {
        return { type: 'bold', text: fullBold[1] };
      }
      // Inline bold: ** appears somewhere but line isn't fully wrapped
      if (line.includes('**')) {
        return { type: 'mixed', text: line };
      }
  ...
  function renderInlineBold(text) {
    // Split on ** pairs; odd-indexed segments are bold, even are plain.
    // If unpaired (odd number of **), fall back to plain escaped text.
    const parts = text.split('**');
    if (parts.length % 2 === 0) {
      // Odd number of ** markers — unclosed; treat entire line as plain
  ...
  function renderPrioritySegments(segments) {
    let html = '';
    let ulOpen = false;
    let inGroup = false;
    for (const seg of segments) {
      if (seg.type === 'bold') {
        ...
        if (inGroup) { html += '</div>'; }
        // Each bold header + its following items form a group with break-inside: avoid.
        // Groups are small (one header + a few bullets), so Chromium won't drop them.
        html += `<div class="priority-group"><p class="priority-bold">${escapeHtml(seg.text)}</p>`;
        inGroup = true;
      } else {
        if (!ulOpen) { html += '<ul class="priority-list">'; ulOpen = true; }
        if (seg.type === 'mixed') {
          html += `<li>${renderInlineBold(seg.text)}</li>`;
        } else {
          html += `<li>${escapeHtml(seg.text)}</li>`;
        }
      }
    }
    if (ulOpen) { html += '</ul>'; }
  ...
  function priorityList(text) {
    const segments = parsePriorityField(text);
    if (!segments) return '<p class="priority-empty">No domains scored in this range.</p>';
    return renderPrioritySegments(segments);
  }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .red-flags-header { background-color: #DC2626; color: #ffffff; }
    ...
    @media print {
      @page {
        ...
        margin: 0.5in 0.5in 0.1in 0.5in;   /* bottom 0.1in matches bottomMargin in renderHtmlToPdf call */
      }
      ...
      .page-wrapper {
        min-height: unset;
        padding-bottom: 50px;   /* footer height (26) + bottom offset (10) + buffer (14) */
      }
    }
  </style>
</head>
  ...
  <div class="two-col">
    ...
    <div class="col-right">
      <div class="priority-section">
        ...
        ${priorityList(mustHaveItems)}
      </div>
      <div class="priority-section">
        <div class="priority-header nice-to-have-header">Nice to Have</div>
        ${priorityList(niceToHaveItems)}
      </div>
      <div class="priority-section">
        <div class="priority-header red-flags-header">Red Flags</div>
        ${priorityList(redFlagsItems)}
      </div>
    </div>
...
export async function createRubricPdf({
  ...
  const mustHaveItems   = mustHaveField   || '';
  const niceToHaveItems = niceToHaveField || '';
  const redFlagsItems   = redFlagsField   || '';
  ...
  return renderHtmlToPdf(html, { landscape: true, bottomMargin: '0.1in' });
}
