...
function buildRubricHtml({
  ...
  // Values come from Airtable fields (Claude-generated narrative text,
  // semicolon-separated, editable by the PM before PDF generation).

  function priorityList(text) {
    if (!text || text.trim() === '') {
      return '<p class="priority-empty">No items.</p>';
    }
    const items = text.split(';').map(s => s.trim()).filter(s => s.length > 0);
    if (items.length === 0) return '<p class="priority-empty">No items.</p>';
