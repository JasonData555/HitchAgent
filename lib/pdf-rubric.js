...
import { replaceArrows, parseInlineSegments, renderInlineSegmentsHtml } from './format-parser.js';

// ─────────────────────────────────────────────────────────────────────────────
// Shared helpers
// ─────────────────────────────────────────────────────────────────────────────

/** HTML-escape a string used in structural template slots (labels, metadata). */
function esc(str) {
  return (str || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
function renderBlock(block, dotColor, isFirst) {
  ...
  if (block.heading !== null) {
    ...
    parts.push(`<p class="block-heading"${style}>${renderInlineSegmentsHtml(parseInlineSegments(block.heading))}</p>`);
  }

  for (const plain of (block.plains || [])) {
    parts.push(`<p class="plain-text">${renderInlineSegmentsHtml(parseInlineSegments(plain))}</p>`);
  }
  ...
  for (const item of block.items) {
    parts.push(
      ...
      `<span class="bullet-text">${renderInlineSegmentsHtml(parseInlineSegments(item))}</span>` +
      `</div>`
    );
  }
...
/** Return up to n hyphen-prefixed bullet texts from a content field, truncated to maxLen chars each. */
function firstNBulletTexts(text, n = 2, maxLen = 120) {
  if (!text) return [];
  return text.split(/\r?\n/)
    .filter(l => l.trim().startsWith('-'))
    .slice(0, n)
    .map(l => {
      const t = replaceArrows(l.trim().replace(/^-\s*/, ''));
...
const HTML_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .context-value { font-size: 12px; font-weight: 500; color: #111827; }
  ...
  .header-right-flex { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }

  .item-count { font-size: 10px; font-weight: 400; color: #9CA3AF; }

  /* ── Chevron ── */
  .chevron { flex-shrink: 0; transition: transform 0.2s ease; }
  .accordion-item[data-open] .chevron { transform: rotate(180deg); }

  /* ── Accordion Preview (collapsed state) ── */
  .accordion-preview {
    display: flex;
    flex-direction: column;
    gap: 4px;
  ...
  .preview-dot { width: 5px; height: 5px; border-radius: 50%; flex-shrink: 0; margin-top: 6px; opacity: 0.4; }

  .preview-text { font-size: 11px; color: #9CA3AF; font-style: italic; line-height: 1.5; }
  ...
  /* ── Info links (markdown [anchor](url) in content fields) ── */
  .info-link { color: #0F2D52; text-decoration: underline; }

  /* ── Side-by-side NTH + RF row ── */
  .side-by-side-row {
    display: flex;
    gap: 10px;
    margin-bottom: 10px;
  ...
  .side-by-side-row > .accordion-item {
    flex: 1;
    margin-bottom: 0;
    min-width: 0;
  }
  ...
  @media print {
    ...
    .side-by-side-row .accordion-body {
      max-height: none !important;
      overflow: visible !important;
    }
    ...
    .accordion-body-inner { break-inside: avoid; page-break-inside: avoid; }
  }
`;
...
function renderAccordionSection({ key, label, accentColor, content, isDefaultOpen, hasPreview }) {
  ...
  let previewHtml = '';
  if (hasPreview) {
    const bullets = firstNBulletTexts(content, 1);
    const rows = bullets.length > 0
      ? bullets.map(b =>
          `<div class="preview-bullet-row">` +
          `<span class="preview-dot" style="background:${accentColor}"></span>` +
          `<span class="preview-text">${esc(b)} — click to expand</span>` +
          `</div>`
        ).join('\n      ')
      : `<div class="preview-bullet-row"><span class="preview-text" style="color:#9CA3AF">click to expand</span></div>`;
    previewHtml = `
...
export function buildRubricHtml({
  ...
  const sections = [
    { key: 'fr',  label: 'Functional Responsibility', accentColor: '#0F2D52', content: functionalResponsibility, isDefaultOpen: true,  hasPreview: true },
    { key: 'sr',  label: 'Success in Role',            accentColor: '#0F2D52', content: successInRole,           isDefaultOpen: false, hasPreview: true },
    { key: 'mh',  label: 'Must Have',                  accentColor: '#166534', content: mustHave,                isDefaultOpen: true,  hasPreview: true },
  ...
  const [frHtml, srHtml, mhHtml, nthHtml, rfHtml] = sections.map(renderAccordionSection);
  const accordionHtml =
    frHtml + '\n' +
    srHtml + '\n' +
    mhHtml + '\n' +
