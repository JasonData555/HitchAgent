...
function buildRubricHtml({
  ...
  // Values come from Airtable fields (pre-computed at draft generation time,
  // editable by the PM before PDF generation). Do not recalculate here.
  const mustHave     = mustHaveItems;
  const niceToHave   = niceToHaveItems;
  const notImportant = notImportantItems;
...
export async function createRubricPdf({
  ...
  // Parse comma-separated Airtable field values into arrays for the priority sections.
  // "None" and empty values both produce an empty array (renders as "No domains scored in this range.").
  function parseFieldList(fieldValue) {
    if (!fieldValue || fieldValue.trim() === '' || fieldValue.trim().toLowerCase() === 'none') return [];
    return fieldValue.split(',').map(s => s.trim()).filter(s => s.length > 0);
  }

  const mustHaveItems     = parseFieldList(mustHaveField);
  const niceToHaveItems   = parseFieldList(niceToHaveField);
  const notImportantItems = parseFieldList(notImportantField);
