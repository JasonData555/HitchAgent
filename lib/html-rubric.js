...
export function createRubricHtml({
  ...
  // ── Split content at second bullet: preview (first two items) + extra (rest) ─
  function splitPreviewTwo(text) {
    ...
    // Fewer than 2 bullets — all content is preview, nothing extra
    if (splitIdx === -1) return { previewText: text, extraText: '' };

    // Lines up to and including the second bullet → preview
    // Lines after the second bullet → extra
    const previewLines = lines.slice(0, splitIdx + 1);
    const extraLines   = lines.slice(splitIdx + 1);
  ...
  function buildSection({ title, accentClass, content, skipIfEmpty, emptyMsg, dataCol }) {
    ...
    if (!hasContent) {
      ...
      const { previewText } = splitPreviewTwo(content);
      const previewHtml  = parseFieldToHtml(previewText);
      const fullHtml     = parseFieldToHtml(content);
      const previewItems = countBullets(previewText);
      const hiddenItems  = bulletCount - previewItems;

      let moreLessBtn = '';
      if (hiddenItems > 0) {
        const btnLabel = `View ${hiddenItems} more item${hiddenItems !== 1 ? 's' : ''} \u2192`;
        moreLessBtn = `<button class="more-less-btn"
        data-title="${esc(title)}"
        data-count="${bulletCount}"
        data-accent="${esc(accentClass)}"
        data-content="${esc(fullHtml)}">${btnLabel}</button>`;
      }
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .modal-body li:last-child { padding-bottom: 0; }
    ...
    .modal-body .bold-label:first-child { margin-top: 0; }
...
<div class="modal-overlay" id="rubric-modal" role="dialog" aria-modal="true" aria-label="">
  <div class="modal-panel" id="rubric-modal-panel">
    <div class="modal-header">
      <div class="modal-header-left">
        <span class="modal-label" id="modal-label-text"></span>
        <span class="modal-count" id="modal-count-text"></span>
      </div>
...
(function () {
  ...
  var overlay    = document.getElementById('rubric-modal');
  var panel      = document.getElementById('rubric-modal-panel');
  var labelEl    = document.getElementById('modal-label-text');
  var countEl    = document.getElementById('modal-count-text');
  var bodyEl     = document.getElementById('modal-body-content');
  var closeBtn   = document.getElementById('modal-close-btn');
  var triggerBtn = null;
  var escListener = null;

  function accentBulletColor(accentClass) {
    var color = ACCENT_COLORS[accentClass] || '#6B7280';
  ...
  function openModal(btn) {
    ...
    countEl.textContent = count ? count + ' item' + (count !== '1' ? 's' : '') : '';

    bodyEl.innerHTML = content;

    var borderColor = ACCENT_COLORS[accent] || '#6B7280';
    panel.style.borderLeft = '3px solid ' + borderColor;

    var existingStyle = bodyEl.querySelector('style.modal-bullet-style');
    if (existingStyle) existingStyle.remove();
    var styleTag = document.createElement('style');
    styleTag.className = 'modal-bullet-style';
    styleTag.textContent = '#modal-body-content li::before { background: ' + accentBulletColor(accent) + '; }';
  ...
  document.querySelectorAll('.more-less-btn').forEach(function (btn) {
