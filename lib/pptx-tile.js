 ...
 *  │ LinkedIn ├───────────────────────────────────────────────────────────┤ ← gray divider
 *  │ SITUATION│  REASONS TO CONSIDER (10pt bold)             ROW 2 (full)│
 *  │ {text}   │  • bullet 1   • bullet 2   • bullet 3                    │
 *  │ CONTACT  ├─────────────────────────┬─────────────────────────────────┤ ← gray divider
 *  │ INFO     │ CULTURE ADD             │ ANTICIPATED CONCERNS   ROW 3    │
 *  │ email    │ {value}                 │ {value}                         │
 *  │ Location ├─────────────────────────┴─────────────────────────────────┤ ← blue footer y:7.1
...
export async function createCandidateTilePresentation(data) {
  ...
  const strReasons   = stripMarkdown(reasonsToConsider) || '';
  ...
  // Right column: count explicit newlines (Claude formats RDE line-by-line)
  const expertiseLines = Math.max(1, (relevantDomainExpertise || '').split('\n').length);
  const reasonsLines   = Math.max(1, strReasons.split('\n').length);

  // Project right column bottom; if it would exceed 6.8" before footer, reduce font
  {
    const eH = 0.25 + (expertiseLines * lineH);
    const rH = 0.2  + (reasonsLines  * lineH);
    const proj = 0.9 + eH + 0.25 + rH + 0.2 + 0.25 + 0.5;
    if (proj > 6.8) {
      bodyFontSize = 8;
      lineH = 0.14;
    }
  ...
  const finalReasonsHeight   = 0.2  + (reasonsLines  * lineH);
  ...
  const reasonsY          = 0.9 + finalExpertiseHeight + 0.25;
  const expertiseContentH = Math.max(0.5, expertiseLines * lineH);
  const reasonsContentH   = Math.max(0.22, reasonsLines * lineH);
  // cultureY: measured from actual Reasons bottom (reasonsY + 0.24 + content) + 0.3" gap
  const cultureY          = reasonsY + 0.24 + reasonsContentH + 0.3;
  ...
  slide.addText([
    ...
    { text: strConcerns, options: { bold: false, color: SLATE, fontSize: bodyFontSize } },
  ], {
    x: RIGHT_X,
    y: concernsY,
    w: RIGHT_W,
