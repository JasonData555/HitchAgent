...
export async function createCandidateTilePresentation(data) {
  ...
  // Left column Y positions — cascade from situation height
  // ~45 chars/line at 10pt Calibri in a 3" wide text box
  const situationLines    = Math.ceil((strSituation.length || 1) / 45);
  const situationHeight   = 0.2 + (situationLines * 0.18); // 0.2" header + content lines
  const situationContentH = Math.max(0.3, situationLines * 0.18);
  const contactInfoY      = 3.4 + situationHeight + 0.3;
  const emailY            = contactInfoY + 0.25;
  const locationY         = emailY + 0.25;
  const educationHeaderY  = locationY + 0.25;
  ...
  const cultureY        = reasonsY + finalReasonsHeight + 0.2;
  const concernsY       = cultureY + 0.25;
