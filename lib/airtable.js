 ...
 * @param {{ typecast?: boolean }} [options] - typecast:true lets Airtable auto-create
 *   single-select/collaborator options for values that don't yet exist.
 * @returns {Promise<{ id: string, fields: object, createdTime: string }>}
 */
export async function updateRecord(table, recordId, fields, { typecast = false } = {}) {
  const url = `${BASE_URL()}/${encodeURIComponent(table)}/${recordId}`;
