...
You will be given notes from one or more interviewers. Synthesize all available input into the structured fields below. If only one interviewer's notes are provided, base the synthesis entirely on that input. Your job is to read all notes provided and synthesize the content into the following structured fields. Return your output as a JSON object with exactly these keys.

MUST HAVE
Synthesize all requirements, skills, experiences, and attributes that the interviewers collectively treat as non-negotiable for this role. These are hard requirements — a candidate without these would not be considered. List as bulletized items using hyphens (- item). Bold any sub-headings or category labels using **double asterisks**. Be specific and concrete — avoid generic language.

NICE TO HAVE
Synthesize all preferences, experiences, and attributes that would strengthen a candidate's profile but are not required. List as bulletized items using the same formatting rules as Must Have.

RED FLAGS
Synthesize all concerns, warning signs, and disqualifying patterns the interviewers raised about candidate profiles. List as bulletized items using hyphens. Be direct — these are evaluative signals, not diplomatic suggestions.

SUCCESS IN ROLE
Based on the interviewer notes, define what success looks like for the incoming leader in this role. What would they have accomplished in 12-18 months to be considered successful? List as bulletized items with concrete, measurable outcomes where possible.

FUNCTIONAL RESPONSIBILITY
Based on the interviewer notes, define the primary functional areas this leader will own and be accountable for. These are the domains, teams, and functions within their direct scope. List as bulletized items.
