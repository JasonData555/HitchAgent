...
Use these as context for the Rubric Match, Anticipated Concerns, and Culture Add sections as instructed below.
  ...
  const reasonsCalibration = rubricItemsBlock
    ? `
STRUCTURE — exactly two paragraphs, separated by one blank line, and nothing else:

Paragraph 1 begins with the literal characters "**Must Have —** " and covers ONLY the must_have rows of the Rubric Match table.
Paragraph 2 begins with the literal characters "**Nice to Have —** " and covers ONLY the nice_to_have rows.

Never write a third paragraph. Never write an unlabeled paragraph. Gaps do not get their own paragraph — a Must Have gap belongs inside paragraph 1, a Nice to Have gap inside paragraph 2.

LENGTH — paragraph 1 is at most 6 sentences. Paragraph 2 is at most 3 sentences. This is a hard limit. The table may hold thirty rows; the narrative must not. Cutting detail to fit is expected and correct.

SYNTHESIZE, DO NOT ENUMERATE. Group the rows of each priority into a few themes — the areas where the candidate is strong, and the areas where coverage is thin — and write one sentence per theme, not one sentence per row. Several not_found rows in the same theme collapse into a single clause naming the gap.

Within each paragraph, order the sentences: strongest matches first, then partial matches, then the material gaps last. Translate the verdict you assigned each row into graded language:
- evidenced: assert the match. Name the company or role and one concrete detail — team size, scope, or outcome.
- inferred: hedge it. Say what supports the inference and what is missing ("adjacent", "less directly proven", "implied but not stated").
- not_found: say plainly that it is not evidenced in the materials reviewed. Never soften a not_found into a match, and never invent evidence to cover one.

Rules:
- Exclude red_flag rows entirely — they belong to Anticipated Concerns, not here.
- Omit paragraph 2 entirely if the Rubric Match table has no nice_to_have rows. In that case output paragraph 1 alone.
- FORBIDDEN WORDS in the prose: rubric, panel, interviewer, score, priority, not_found. The reader must never learn that a scoring artifact exists. The only permitted use of "Must Have" and "Nice to Have" is as the two bold paragraph labels themselves. ("evidenced" and "inferred" are fine as ordinary English verbs — "his compliance work is not evidenced in the materials reviewed" — just never as verdict labels.)
- Do not name the source documents or quote them. Write "no evidence of X in the materials reviewed", never "the recruiter notes say" or "per the resume", and never quote a recruiter or reference verbatim. State the observation directly.
- Prose sentences only — no bullets, no hyphen or • prefixes, no per-item bold labels inside the paragraphs.`
    : `
No Rubric items were provided for this candidate, so there are no Must Have or Nice to Have requirements to assess against. Write a single unlabeled paragraph of 3–5 sentences covering the candidate's strongest and most differentiating experience, grounded in the resume, notes, and LinkedIn data. Name companies and concrete details — team size, scope, outcome. Prose sentences only — no bullets, no paragraph labels.
...
Respond in this exact JSON format:
{
    ...
    "reasonsToConsider": "**Must Have —** Eight years at Coinbase directly evidence enterprise security leadership at scale: she built and ran a 300-person org spanning cloud, infrastructure, and product security. Board-level communication is equally well supported — she presented quarterly to Kudelski's audit committee. FedRAMP authorization is less directly proven; her compliance work is adjacent (SOC 2, ISO 27001) but no federal authorization program is named.\\n\\n**Nice to Have —** Fintech regulatory exposure is strong via Coinbase's NYDFS regime. A prior CISO-of-record title is not evidenced in the materials reviewed.",
