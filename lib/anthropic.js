 ...
 *
 * Model per task — see TILE_MODEL / RUBRIC_MODEL / PORTAL_MODEL below.
 * synthesizeCandidateContent() → { situation, relevantDomainExpertise, rubricMatch,
 *                                  reasonsToConsider, cultureAdd, anticipatedConcerns }
 *
 * `rubricMatch` is an INTERNAL working step, not an output. Claude still assigns a
 * verdict to every rubric item — Reasons to Consider and Anticipated Concerns are
 * both calibrated on those verdicts — but the table is neither written to Airtable
 * nor rendered on any tile. It was redundant with Reasons to Consider and cluttered
 * the layout. Callers should ignore the key; do not reintroduce a write for it.
 */

import Anthropic from '@anthropic-ai/sdk';
import { log } from './logger.js';

// Model per task. Each call site names the one it wants — there is no shared
// "default model", because the three paths are tuned for different constraints.
//
// TILE_MODEL is Sonnet 5 rather than Opus 5 on purpose, and NOT to save money.
// The tile runs on api/generate-draft.js under a hard 300s Vercel maxDuration,
// with a 135s per-request timeout that must leave room for a retry. Measured on
// a shape-matched prompt: Sonnet 5 sustains ~109 output tok/s vs Opus 5's ~76.
// Extrapolated to the largest real record (~12k output tokens incl. thinking)
// that is ~110s on Sonnet 5 (fits) vs ~158s on Opus 5 (blows the timeout, and
// raising the timeout to compensate pushes 2 attempts past the 300s budget).
// Re-measure before moving this path to a slower model.
export const TILE_MODEL = 'claude-sonnet-5';

// Rubric is ~3 calls/month with small output, so it carries no latency pressure
// and takes the strongest available model. Its output feeds every downstream
// tile, which is why it is not left on a cheaper tier.
export const RUBRIC_MODEL = 'claude-opus-5';

// ⚠️ PORTAL STAYS ON SONNET 4.6 — a deliberate hold, not an oversight.
// callPortalClaudeWebSearch() concatenates every text block and JSON.parses the
// result, with no extraction or repair step. That is too fragile for a thinking
// model, and both frontier candidates were measured failing it on the Market
// Intelligence prompt:
//   claude-opus-5     -> emits a conversational preamble ("I'll research...")
//                        before the searches, so the join yields prose + JSON
//   claude-sonnet-5   -> emits a raw control character inside a JSON string
//                        ("Bad control character in string literal")
//   claude-sonnet-4-6 -> clean
// The JD call (no web search) did pass on Opus 5, but it shares the same
// unhardened parser and the Sonnet 5 failure mode could surface there on any
// run — not worth the risk on client-facing content generated ~3x/month.
// To upgrade this path, first harden the parser to extract the JSON document
// from surrounding prose, then re-test both models.
const PORTAL_MODEL = 'claude-sonnet-4-6'; // client-facing MI/JD (generate-portal)

// Ceiling for the rubric-synthesis and portal MI/JD calls. The tile path no
// longer uses this — see TILE_MAX_TOKENS below. Raised from 6000 because
// thinking tokens count against max_tokens on these models, not just toward
// billing, so the old ceiling no longer leaves room for the JSON itself.
const MAX_TOKENS = 10000;

// The tile prompt is the only one whose output scales with BOTH the candidate's
// career length and the linked Rubric's item count, so it gets its own ceiling.
// A 53-item rubric (25/16/12) plus an eight-company tenure list needs ~7-8k
// tokens: the internal Rubric Match table alone is one row per rubric bullet,
// and it is emitted BEFORE the three sections it feeds, so overflow silently
// eats Reasons to Consider / Culture Add / Anticipated Concerns. At 6000 that
// truncated mid-string and surfaced as "Unterminated string in JSON".
// This is a ceiling, not a target — a typical candidate still emits ~3-4k
// tokens at unchanged latency.
// Raised again to 24000 for Sonnet 5: thinking tokens are billed as output AND
// consume max_tokens, so the 16000 that fit Haiku's non-thinking output would
// reintroduce the same truncation. The stop_reason === 'max_tokens' guard below
// will say so loudly if this estimate is still short.
const TILE_MAX_TOKENS = 24000;

// Per-request timeout for a single Claude call. The dispatcher function budget
// is 300s (Vercel Pro); keeping each attempt at 135s lets the retry-once-on-
// timeout wrappers fire a second attempt (2 x 135s = 270s < 300s, leaving ~30s
// for the pre-Claude fetches and the Airtable write) and still surface a
// catchable error INSIDE the function budget — so a genuinely stuck generation
// writes "Draft Error" instead of being hard-killed by the platform (which
// bypasses every catch and leaves the record silently at "Not Started").
// Do NOT raise past ~145s without revisiting maxDuration in vercel.json.
const CLAUDE_TIMEOUT_MS = 135000;
 ...
 *
 * The resulting item titles feed the Rubric Match verdict table, which is an
 * INTERNAL working step only — it is never written to Airtable and never rendered.
 * Its verdicts are what Reasons to Consider and Anticipated Concerns are built from.
 *
...
THIS SECTION IS MATCHES ONLY. It answers "where does this candidate align?" — never "where does he fall short?". Gaps, shortfalls, missing evidence, and anything needing verification belong exclusively to Anticipated Concerns (section 6), which is built from the same verdicts. Do not preview them here, not even as a trailing caveat inside an otherwise positive bullet.

Build the bullets ONLY from must_have and nice_to_have rows you marked evidenced or inferred. Order them strongest first. Translate the verdict into the bullet's language:
- evidenced: assert it. Name the company and one concrete detail — team size, scope, or outcome.
- inferred: assert the alignment that IS supported, and stop there. State what the background shows, not what it lacks.

Rules:
- Exclude every not_found row. A requirement with no evidence is a gap; it is reported in Anticipated Concerns and is silently absent from this section. Never write a bullet whose point is that something was not found.
- Exclude red_flag rows entirely — they belong to Anticipated Concerns, not here.
- No bullet may contain a "but", "though", "however", "pending", "requires verification", "not detailed", "not specified", or any other clause that qualifies the match away. If a theme cannot be stated as a clean positive, drop the bullet rather than hedge it. Fewer, stronger bullets beat padded ones — it is fine to write only two.
- Omit the "**Nice to Have**" heading and its bullets entirely if no nice_to_have row is evidenced or inferred.
...
- FORBIDDEN WORDS in the bullet text: rubric, panel, interviewer, score, priority, not_found. The reader must never learn that a scoring artifact exists. The only permitted use of "Must Have" and "Nice to Have" is as the two heading lines themselves.
- Do not name the source documents or quote them ("the recruiter notes say", "per the resume"). State the observation directly.
...
3. RUBRIC MATCH CRITERIA TABLE — INTERNAL WORKING STEP

This table is NOT shown to anyone. It is not stored and it is not rendered on the tile. It exists so that you commit to a verdict on every requirement BEFORE you write Reasons to Consider (section 4) and Anticipated Concerns (section 6) — both of those sections are built directly from the verdicts you assign here. Do the work honestly; it is the evidence base for everything downstream.
...
NOTES RULES:
One fragment only, 12 words MAXIMUM. This is a working note, not prose — no full
sentences, no lead-ins, no restating the item title. Stay under 12 words even when
the evidence is rich; the verdict carries the weight, the note is only the pointer.
evidenced: name the source (resume, notes, LinkedIn) and the specific evidence.
inferred: name the indirect evidence and what is missing.
not_found: "No evidence found; verify in screening call."
...
4. REASONS TO CONSIDER (bulleted alignment summary)
Where the candidate ALIGNS with the requirements for this search. Positive evidence only — this is the case for the candidate, not an assessment of him.
...
- Never overstate. If the evidence does not support a claim, omit the claim — do not stretch it, and do not qualify it with what is missing. Gaps are reported in Anticipated Concerns.
...
Respond in this exact JSON format:
{
    ...
    "rubricMatch": "Board experience | must_have | evidenced | Resume: board presentation role at Kudelski\\nFISMA/FedRAMP compliance | must_have | inferred | Resume: FedRAMP-adjacent work; no authorization program stated",
    "reasonsToConsider": "**Must Have**\\n- **Security leadership at scale:** Built and ran a 300-person security org at Coinbase spanning cloud, infrastructure, and product.\\n- **Board communication:** Presented quarterly to Kudelski's audit committee.\\n- **Cloud security depth:** Owned AWS and GCP security architecture across a 40,000-node fleet.\\n\\n**Nice to Have**\\n- **Fintech regulatory exposure:** Owned NYDFS compliance at Coinbase.",
...
/**
 * Concatenate the text blocks of a response, skipping every non-text block.
 *
 * `message.content[0]` is NOT reliably the text. Thinking-capable models emit a
 * `thinking` block first whenever adaptive thinking engages — which, on a prompt
 * this size, is essentially always — so reading `content[0].text` yields
 * undefined and the JSON.parse below dies with a misleading SyntaxError. Filter
 * by type instead. Single-text-block responses are unaffected: the join returns
...
async function callClaude(prompt) {
  const message = await client.messages.create({
    ...
    // Default effort is "high". "medium" keeps thinking spend proportionate for
    // what is ultimately a well-specified extraction-and-synthesis task, and is
    // the setting the latency figures above were measured at. Do NOT swap this
    // for thinking:{type:'disabled'} — on these models that leaks tool calls and
    // <thinking> tags into the visible response; lowering effort is the lever.
    output_config: { effort: 'medium' },
    messages: [{ role: 'user', content: prompt }],
  }, { timeout: CLAUDE_TIMEOUT_MS });

  // Catch budget exhaustion BEFORE JSON.parse. A truncated response fails as
  // "Unterminated string in JSON at position N" — a SyntaxError, which the
  // retry wrapper in synthesizeCandidateContent() treats as transient and
  // re-runs. But truncation is deterministic: the identical prompt at the
  // identical budget truncates identically, so the retry only burns another
  // full generation before failing the same way. Throwing a non-SyntaxError
  // here both names the real cause in the log and skips that wasted attempt.
  if (message.stop_reason === 'max_tokens') {
    const used = message.usage?.output_tokens ?? TILE_MAX_TOKENS;
    log('claude_truncated', { stopReason: message.stop_reason, outputTokens: used, maxTokens: TILE_MAX_TOKENS });
    throw new Error(`Claude response truncated at max_tokens (${used} of ${TILE_MAX_TOKENS} tokens)`);
  }
...
export async function generateRubricPriorityText(clientName, searchName, panelNotes, domainTiers) {
  ...
  async function attempt() {
    // DEAD CODE — zero callers. Kept only because the export still exists.
    // NOTE: max_tokens 800 is not viable on a thinking model (thinking consumes
    // max_tokens), so this would truncate immediately if it were ever revived.
    // Raise the ceiling before reusing it; it is left as-is to avoid touching
    // generation behaviour in a model-configuration change.
    const message = await client.messages.create({
      model: RUBRIC_MODEL,
      max_tokens: 800,
      messages: [{ role: 'user', content: prompt }],
...
Each value is a plain text string using hyphens for bullet items and **double asterisks** for bold labels. Do not return markdown code blocks, preamble, or explanation — only the JSON object.`;
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      messages: [{ role: 'user', content: prompt }],
      // Without an explicit timeout this inherits the SDK's 10-minute default,
      // which is far longer than the 300s lambda budget — a slow call would be
      // hard-killed by the platform, bypassing every catch and leaving the
      // record silently at "Not Started" with no Draft Error.
    }, { timeout: CLAUDE_TIMEOUT_MS });
    const rawText = extractText(message);
    const cleaned = stripCodeFences(rawText);
    const data = JSON.parse(cleaned);
...
export async function synthesizeCandidateContent(
  ...
  try {
    ...
    // Retry once on timeout, transient error, or malformed JSON. NOT on a
    // max_tokens truncation — callClaude() throws a plain Error for that, so it
    // falls through to the rethrow below. Truncation is deterministic; retrying
    // it just spends a second full generation to fail identically.
    if (
      firstError instanceof SyntaxError ||
      firstError.message?.includes('timeout') ||
      firstError.message?.includes('Missing or invalid field') ||
...
async function callPortalClaude(systemPrompt, userPrompt, requiredKeys) {
  const message = await client.messages.create({
    ...
    // Explicit timeout for the same reason as the rubric call above: the SDK
    // default (10 min) outlives the 300s lambda budget.
  }, { timeout: CLAUDE_TIMEOUT_MS });
  ...
  for (let turn = 0; turn < 5; turn++) {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: systemPrompt,
        messages,
        // Keep web_search_20250305 — verified against the current model. The
        // newer web_search_20260209 variant runs code execution under the hood
        // and returns many interleaved text / code_execution_tool_result blocks,
        // which would break the single-JSON-document parse below.
        tools: [{ type: 'web_search_20250305', name: 'web_search', max_uses: maxUses }],
      }),
      // Per-request bound; raw fetch otherwise waits indefinitely. Note this is
      // per turn — the pause_turn loop below can still run several turns, so the
      // 300s lambda budget, not this value, is the real ceiling on the total.
