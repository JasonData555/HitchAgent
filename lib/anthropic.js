 ...
 * synthesizeCandidateContent() → { situation, relevantDomainExpertise, rubricMatch, cultureAdd, anticipatedConcerns }
 */
 ...
 * Only hyphen-prefixed bullet lines ("- item text") become table rows.
 *
 * Title extraction rules (applied in order):
 *   1. Truncate at first semicolon
 *   2. Truncate at first em dash (—)
 *   3. Truncate at first ": " (colon-space) with remaining text
 *   4. Trim
 *   5. Truncate at last word boundary before 55 characters
 *
...
export function extractRubricItemTitles(fieldValue, priority) {
  ...
  for (const raw of fieldValue.split('\n')) {
    ...
    // Only process bullet items (hyphen-space prefix)
    if (!line.startsWith('- ')) continue;

    let text = line.slice(2).trim(); // strip "- " prefix

    // Rule 1: truncate at first semicolon
    const semiIdx = text.indexOf(';');
    if (semiIdx !== -1) text = text.slice(0, semiIdx);

    // Rule 2: truncate at first em dash
    const emIdx = text.indexOf('—');
    if (emIdx !== -1) text = text.slice(0, emIdx);

    // Rule 3: truncate at first ": " (colon followed by space + more text)
    ...
    // Rule 5: truncate at last complete word before 55 characters
    if (text.length > 55) {
...
Populate each entry's details using the source precedence defined in SOURCE HARNESS: Resume and Notes are authoritative for company names, role titles, dates, scope, team size, and accomplishments; LinkedIn fills gaps only and never overrides them. Where the recruiter notes carry qualitative observations or reference feedback that contextualizes a tenure, incorporate it naturally into that entry's description.`;
  ...
  const anticipatedConcernsInstructions = (mustHave || redFlags)
    ? `4. ANTICIPATED CONCERNS (2-3 items)
...
async function callClaude(prompt) {
  ...
  const REQUIRED = ['situation', 'relevantDomainExpertise', 'rubricMatch', 'cultureAdd', 'anticipatedConcerns'];
 ...
 * @returns {Promise<{ situation: string, relevantDomainExpertise: string, rubricMatch: string, cultureAdd: string, anticipatedConcerns: string }>}
 */
