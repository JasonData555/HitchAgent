 ...
 * @param {{ landscape?: boolean, bottomMargin?: string, footerTemplate?: string }} [options]
 *   footerTemplate — optional HTML rendered by Chromium inside the bottom page
 *   margin on EVERY page. Unlike a `position: fixed` footer in the document, the
 *   margin band is space the paginator will not flow content into, so body text
 *   can never render behind it. Omit to keep the plain document-only behaviour.
 * @returns {Promise<Buffer>} PDF binary data
 */
export async function renderHtmlToPdf(htmlString, { landscape = false, bottomMargin = '0.5in', footerTemplate = null } = {}) {
  const executablePath = await getExecutablePath();

  const browser = await puppeteer.launch({
    args: isLocalDev ? LOCAL_CHROME_ARGS : chromium.args,
    // Match Letter size at 96dpi — swap dimensions for landscape.
  ...
  try {
    ...
    pdf = await page.pdf({
      ...
      // headerTemplate must be non-empty when displayHeaderFooter is on, or
      // Chromium falls back to its built-in title/date header.
      ...(footerTemplate && {
