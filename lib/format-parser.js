 ...
 *   (blank line)    — extra vertical space before the next block (parseFormattedText/
 *                      renderFormattedHtml only; consecutive blank lines collapse to one)
 *
 * Exports:
 ...
 *
 * Blank / whitespace-only lines are skipped, but a blank line still has a
 * visual effect: it marks the next line with `spacer: true` so the renderer
 * can add extra space before it (consecutive blank lines collapse to one).
 *
 * @param {string} rawText
 * @returns {{ type: 'heading'|'bullet'|'plain', raw: string, content: string,
 *             segments: object[], spacer: boolean }[]}
 */
export function parseFormattedText(rawText) {
  ...
  for (const line of rawText.split('\n')) {
    ...
    if (!trimmed) {
      if (result.length > 0) pendingSpacer = true;
      continue;
    }
    ...
    if (/^\*\*.+\*\*$/.test(trimmed)) {
      ...
      result.push({ type: 'heading', raw: trimmed, content, segments: parseInlineSegments(content), spacer });
      continue;
    }
    ...
    if (/^[•○\-](\s|$)/.test(trimmed)) {
      ...
      result.push({ type: 'bullet', raw: trimmed, content, segments: parseInlineSegments(content), spacer });
      continue;
    }

    // Plain paragraph
    result.push({ type: 'plain', raw: trimmed, content: trimmed, segments: parseInlineSegments(trimmed), spacer });
  }
...
export function renderFormattedHtml(parsedLines, options = {}) {
  ...
  for (const line of parsedLines) {
    if (line.type === 'heading' && allowHeadings) {
      closeList();
      const cls = line.spacer ? 'block-heading spacer-before' : 'block-heading';
      parts.push(`<p class="${cls}">${renderInlineSegmentsHtml(line.segments)}</p>`);
    } else if (line.type === 'bullet') {
      if (!inList) {
        const classes = [listClass, line.spacer ? 'spacer-before' : ''].filter(Boolean).join(' ');
      ...
      closeList();
      const cls = line.spacer ? ' class="spacer-before"' : '';
      parts.push(`<p${cls}>${renderInlineSegmentsHtml(line.segments)}</p>`);
    }
  }
...
export function parseConcernsItems(text) {
  ...
  // New format: lines starting with -, •, or ○
