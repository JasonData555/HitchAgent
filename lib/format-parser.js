...
export function parseFormattedText(rawText) {
  // [PARSER DEBUG 3] — remove before next production deploy
  console.log('[PARSER DEBUG 3] parseFormattedText called with:', JSON.stringify(rawText?.substring(0, 200)));
  if (!rawText) return [];
  const result = [];
  ...
  // [PARSER DEBUG 4] — remove before next production deploy
  console.log('[PARSER DEBUG 4] parseFormattedText output:', JSON.stringify(result.slice(0, 3)));
  return result;
}
...
export function renderFormattedHtml(parsedLines, options = {}) {
  // [PARSER DEBUG 5] — remove before next production deploy
  console.log('[PARSER DEBUG 5] renderFormattedHtml called with:', JSON.stringify(parsedLines?.slice(0, 3)));
