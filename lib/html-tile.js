...
function expertiseToHtml(text) {
  ...
  for (const line of lines) {
    ...
    if (isCompanyHeader(trimmed)) {
      ...
      // Company name (header) is plain-escaped; desc goes through inline renderer.
...
export async function createCandidateTileHtml({
  ...
  const cultureHtml    = renderInlineSegmentsHtml(parseInlineSegments(cultureAdd || ''));
  ...
  <div class="body">
    ...
    <aside class="sidebar">
      ...
      <div class="section situation">
        ...
        <div class="section-body"><p>${renderInlineSegmentsHtml(parseInlineSegments(situation || ''))}</p></div>
      </div>
