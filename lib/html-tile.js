...
/**
 * Convert a plain-text bullets string into an HTML unordered list.
 * Non-bullet lines are wrapped in <p> tags.
 * Delegates to shared parser so all inline formatting (bold, italic,
 * underline, links) is supported within bullet and paragraph text.
 */
function bulletsToHtml(text) {
  if (!text) return '';
  return renderFormattedHtml(parseFormattedText(text), {});
}
...
export async function createCandidateTileHtml({
  ...
  // [PARSER DEBUG 6] — Final HTML for each field (remove before next production deploy)
  const situationHtmlDebug = renderFormattedHtml(parseFormattedText(situation || ''), {});
  console.log('[PARSER DEBUG 6] situation HTML:', situationHtmlDebug?.substring(0, 500));
  console.log('[PARSER DEBUG 6] relevantDomainExpertise HTML:', expertiseHtml?.substring(0, 500));
  console.log('[PARSER DEBUG 6] reasonsToConsider HTML:', reasonsHtml?.substring(0, 500));
  console.log('[PARSER DEBUG 6] cultureAdd HTML:', cultureHtml?.substring(0, 500));
  console.log('[PARSER DEBUG 6] anticipatedConcerns HTML:', concernsHtml?.substring(0, 500));
