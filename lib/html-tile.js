...
/**
 * Parse Additional Info text and return an HTML string.
 * - Markdown links [text](url) → <a href="url" class="info-link">text</a>
 * - Raw URLs (http:// or https://) not already wrapped → <a href="url" class="info-link">Link</a>
 * - All non-link text is HTML-escaped.
 * - Malformed markdown with empty/non-http URL falls through as plain text.
 */
function parseAdditionalInfoHtml(text) {
  ...
  // Pass 1 — extract markdown links (priority)
  const parts = [];
  let lastIndex = 0;
  let match;
  ...
  // Pass 2 — detect raw URLs within plain text segments
  const finalParts = [];
...
/**
 * Replace Unicode arrow characters with ASCII word equivalents so they
 * render reliably in the Puppeteer/Lambda Chromium environment, which
 * lacks full Unicode Arrows block font coverage.
 */
...
function expertiseToHtml(text) {
  ...
  const isCompanyHeader = (line) => /^[A-Za-z].*\((?:[A-Za-z]{3}\s+)?\d{4}/.test(line.trim());
...
export async function createCandidateTileHtml({
  ...
  const additionalInfoSection = additionalInfo
    ? `<div class="section additional-info">
        ...
        <div class="section-body"><p>${parseAdditionalInfoHtml(additionalInfo)}</p></div>
      </div>`
    : '';
