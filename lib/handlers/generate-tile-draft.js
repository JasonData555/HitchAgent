...
import { synthesizeCandidateContent, extractRubricItemTitles, TILE_MODEL } from '../anthropic.js';
...
export default async function handler(req, res) {
  ...
  // ── Claude synthesis ─────────────────────────────────────────────────────
  let synthesized;
  log('claude_api_called', { model: TILE_MODEL, tileId });
  ...
  // `synthesized.rubricMatch` is deliberately NOT written. Claude still assigns a
  // verdict per rubric item — Reasons to Consider and Anticipated Concerns are both
  // calibrated on those verdicts — but the table is internal reasoning only. It was
  // redundant with Reasons to Consider and cluttered the tile, so it is neither
  // stored in Airtable nor rendered anywhere.
  try {
    await updateRecord(TABLE, tileId, {
