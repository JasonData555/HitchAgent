...
import {
  createCandidateTileHtml,
  TILE_PDF_BOTTOM_MARGIN,
  TILE_PDF_FOOTER_TEMPLATE,
} from '../html-tile.js';
...
export default async function handler(req, res) {
  ...
  let pdfBuffer;
  try {
    // The footer is drawn in the reserved bottom page margin rather than in the
    // document, so body text can never render behind it at a page break.
    pdfBuffer = await renderHtmlToPdf(htmlString, {
      bottomMargin: TILE_PDF_BOTTOM_MARGIN,
      footerTemplate: TILE_PDF_FOOTER_TEMPLATE,
    });
  } catch (err) {
    log('error', { error: err.message, tileId, ...(process.env.NODE_ENV !== 'production' && { stack: err.stack }) });
