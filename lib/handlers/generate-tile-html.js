...
import { getRecord, updateRecord, getFieldValue, getAttachmentUrl } from '../airtable.js';
import { createCandidateTileWebHtml } from '../html-tile-web.js';
...
export default async function handler(req, res) {
  ...
  // A canonical tile_url is never rewritten — it is the link clients already hold.
  // But early records were given a Vercel Blob snapshot or an /api/view proxy of
  // one. Those are frozen at their generation-time content: a PM edit in Airtable
  // can never reach them, and they predate Reasons to Consider entirely. Anything
  // that is not already a /api/tile-view link is therefore migrated to one, which
  // renders live from Airtable on every request.
  const existingUrl = getFieldValue(fields, 'tile_url', '');
  const isCanonical = existingUrl.includes('/api/tile-view');
  const htmlUrl = isCanonical ? existingUrl : permanentUrl;

  log('tile_url_set', { tileId, htmlUrl, isNew: !existingUrl, migrated: !!existingUrl && !isCanonical });

  // ── Write URL back to Airtable ────────────────────────────────────────────
  // Always set Tile Status to Active. Write tile_url unless it is already the
  // canonical live-render URL.
  const fieldsToWrite = { 'Tile Status': 'Active' };
  if (!isCanonical) fieldsToWrite.tile_url = permanentUrl;
